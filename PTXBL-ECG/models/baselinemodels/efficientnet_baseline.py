# -*- coding: utf-8 -*-
"""
EfficientNet基线模型 - ECG分类任务

使用深度可分离卷积和通道注意力机制的1D CNN架构
"""
...
class DepthwiseSeparableConv1d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=False):
        ...
        self.depthwise = nn.Conv1d(in_channels, in_channels, kernel_size, stride, padding, groups=in_channels, bias=bias)
...
class MBConv1D(nn.Module):
    """1D Mobile Inverted Bottleneck卷积块"""
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, expand_ratio, se_ratio=0.25):
        ...
        self.depthwise_conv = nn.Sequential(
            DepthwiseSeparableConv1d(expanded_channels, expanded_channels, kernel_size, stride, 
...
class EfficientNet1DBaseline(nn.Module):
    """1D EfficientNet基线模型"""
    
    def __init__(self, input_channels=12, num_classes=5, dropout=0.3):
        super(EfficientNet1DBaseline, self).__init__()
        ...
        # MBConv块
        self.mbconv1 = MBConv1D(32, 64, 3, 1, 6)
        self.mbconv2 = MBConv1D(64, 128, 3, 2, 6)
        self.mbconv3 = MBConv1D(128, 256, 3, 2, 6)
        self.mbconv4 = MBConv1D(256, 512, 3, 2, 6)
...
def main(args):
    ...
    model = EfficientNet1DBaseline(
        input_channels=12,
        num_classes=len(classes),
        dropout=args.dropout
    ).to(device)
    
    print(f"✅ EfficientNet基线模型已初始化，参数数量: {sum(p.numel() for p in model.parameters()):,}")
    ...
    # 保存结果
    results = {
        'model_type': 'EfficientNet_Baseline',
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='EfficientNet基线模型训练')
    ...
    parser.add_argument('--save_path', type=str, default='./saved_models/efficientnet_baseline_model.pth', help='模型保存路径')
    parser.add_argument('--results_path', type=str, default='./results/efficientnet_baseline_results.npy', help='结果保存路径')
