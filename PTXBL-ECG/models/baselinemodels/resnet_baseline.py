...
class ResNet1DBaseline(nn.Module):
    ...
    def __init__(self, input_channels=12, num_classes=5, dropout=0.3):
        super(ResNet1DBaseline, self).__init__()
...
def main(args):
    ...
    print(f"✅ ResNet基线模型已初始化，参数数量: {sum(p.numel() for p in model.parameters()):,}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='ResNet基线模型训练')
    ...
    parser.add_argument('--save_path', type=str, default='./saved_models/resnet_baseline_model.pth', help='模型保存路径')
    parser.add_argument('--results_path', type=str, default='./results/resnet_baseline_results.npy', help='结果保存路径')
