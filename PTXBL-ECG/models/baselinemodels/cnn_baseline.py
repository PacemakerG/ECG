...
经典的1D CNN架构，包含多个卷积层、批归一化、池化和全连接层
"""
...
def main(args):
    ...
    print(f"✅ CNN基线模型已初始化，参数数量: {sum(p.numel() for p in model.parameters()):,}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='CNN基线模型训练')
    ...
    parser.add_argument('--save_path', type=str, default='./saved_models/cnn_baseline_model.pth', help='模型保存路径')
    parser.add_argument('--results_path', type=str, default='./results/cnn_baseline_results.npy', help='结果保存路径')
