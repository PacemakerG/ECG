# -*- coding: utf-8 -*-
"""
Inception基线模型 - ECG分类任务

使用多尺度卷积的1D CNN架构进行ECG分类
"""
...
class InceptionBlock1D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        # 1x1 卷积分支
        self.branch1x1 = nn.Conv1d(in_channels, out_channels//4, kernel_size=1, bias=False)
        
        # 1x1 -> 3x3 卷积分支
        self.branch3x3 = nn.Sequential(
            nn.Conv1d(in_channels, out_channels//4, kernel_size=1, bias=False),
            nn.Conv1d(out_channels//4, out_channels//4, kernel_size=3, padding=1, bias=False)
        )
        
        # 1x1 -> 5x5 卷积分支
        self.branch5x5 = nn.Sequential(
            nn.Conv1d(in_channels, out_channels//4, kernel_size=1, bias=False),
            nn.Conv1d(out_channels//4, out_channels//4, kernel_size=5, padding=2, bias=False)
        )
        
        # 1x1 -> 7x7 卷积分支
        self.branch7x7 = nn.Sequential(
            nn.Conv1d(in_channels, out_channels//4, kernel_size=1, bias=False),
            nn.Conv1d(out_channels//4, out_channels//4, kernel_size=7, padding=3, bias=False)
        )
...
class Inception1DBaseline(nn.Module):
    """1D Inception基线模型"""
    
    def __init__(self, input_channels=12, num_classes=5, dropout=0.3):
        super(Inception1DBaseline, self).__init__()
...
def main(args):
    ...
    model = Inception1DBaseline(
        input_channels=12,
        num_classes=len(classes),
        dropout=args.dropout
    ).to(device)
    
    print(f"✅ Inception基线模型已初始化，参数数量: {sum(p.numel() for p in model.parameters()):,}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Inception基线模型训练')
    ...
    parser.add_argument('--save_path', type=str, default='./saved_models/inception_baseline_model.pth', help='模型保存路径')
    parser.add_argument('--results_path', type=str, default='./results/inception_baseline_results.npy', help='结果保存路径')
